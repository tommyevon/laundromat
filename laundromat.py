...
class AUTO():
    ...
    class scrape():
            ...
            for deal in raw_text.splitlines():
                ...
                # parse through tranche data, add to details_dict:
                    ...
                    trouble_items = ['bk', 'BK', 'back', 'Mid', 'mid', 'area', 'Area', 'to', 'a',
                                     ...
                                     '2bps', '1bps', '3bps', '4bps', '5bps', 'Low', 'High']
                    ...
                    for item in deal_items_list:
                        ...
                        elif irregular_data_flag is True and index > 14 and item in trouble_items:
        ...
        def Go(self, useless):
            ...
            # grab the deal details from the first listed deal on the website
            try:
                initial_deal_listing = self.format_deal_entry(deal_listings.partition('\n')[0])
            except:
                print('[CLO', self.config['type'], '] format_deal_entry() failure')
                return

            # if the most recent deal does not fall in the desired date range, there are no new deals
            if initial_deal_listing['Issuance_Date'] not in self.time_series:
                print('[CLO', self.config['type'], '] No new deals today, most recent deal issued on', initial_deal_listing['Issuance_Date'])
                self.driver.close()
                return

            # call open_frames to open up all relevant deals on the website
            self.open_frames(deal_listings)

            # grab all of the data off the webiste in str format
            deals = self.grab_deals()
            sleep(2)
            '''
            ...
            try:
                with open(self.config['rawtxt'], 'r') as fin:
                    deals = fin.read()
                    deals = deals.replace('ABS-15G\n', '')
            ...
            # attempt to convert the str format data into a pandas dataframe
            '''
            try:
                master_data_set = self.store_data(deals)
            except:
                print('[CLO', self.config['type'], '] Initial error storing data')
                sleep(5)
                for i in range(3):
                    sleep(5)
                    try:
                        ...
                        print('[CLO', self.config['type'], '] Attempt number', i + 2, 'error store_data')
                        if i == 2:
                            print('[CLO', self.config['type'], '] All store_data attemepts failed')
                            return
                        else:
                            continue
            '''
    ...
    def Go(self):
        ...
        t.start()
        t1 = Thread(target = lambda q, arg1: q.put(moto.Go(arg1)), args=(que, 'junk'))
        t1.start()
        t2 = Thread(target = lambda q, arg1: q.put(floor.Go(arg1)), args=(que, 'junk'))
        t2.start()
        t3 = Thread(target = lambda q, arg1: q.put(lease.Go(arg1)), args=(que, 'junk'))
        t3.start()
        t4 = Thread(target = lambda q, arg1: q.put(subprime.Go(arg1)), args=(que, 'junk'))
        t4.start()
        t5 = Thread(target = lambda q, arg1: q.put(prime.Go(arg1)), args=(que, 'junk'))
        t5.start()
        ...
        odf.to_csv('auto_master_database.csv', index = True)
